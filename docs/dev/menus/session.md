...
| 턴 상세 | 비싼 턴 한 줄을 누르면 **그 줄 바로 아래로** 펼쳐지는 계층. 채팅 / 도구 / MCP / 파일 (아래 절) |

비싼 턴 표와 턴 상세는 `src/views/SessionTurns.jsx` 한 곳에 있고 **프로젝트 화면의 세션 표가 같은 것을**
씁니다(`docs/dev/menus/project.md`). 표를 양쪽에 따로 두면 컬럼·정렬·펼침 규칙이 조용히 갈라져 같은
세션이 두 화면에서 다르게 보입니다. 담는 규칙(토큰 상위 N개)은 화면 없이 검증되도록
`src/session-turns.js` 로 갈라 두었습니다 — `table-sort.js` 와 같은 이유입니다.

곡선과 단계별 배분은 이 화면에만 남깁니다. 프로젝트 화면에서 펼치는 것은 "어느 턴이 비쌌고 그 턴이
무엇에 썼나" 까지이고, 세션 전체의 모양을 보려면 이 화면으로 옵니다.

## 턴 상세: 그 턴이 무엇에 썼나

비싼 턴 표는 "턴 50이 73.9M을 썼다"까지 말합니다. 그 다음 질문은 항상 같습니다 — **73.9M이 어디로 갔나.**

행을 누르면 그 줄 바로 아래로 계층이 펼쳐집니다. 별도 화면이나 모달로 빼지 않은 이유는 "어느 턴의 상세인지"가 표와 붙어 있어야 읽히기 때문입니다.

```text
┌ 비싼 턴 ──────────────────────────────────────────────────────────┐
│ 턴50  08:53  73.9M  88요청  implement  Edit32 Read28        [▾]   │
│ ┌──────────────┬──────────────────────────────────────────────┐  │
│ │   ◯ 원형차트  │ ▾ ● 채팅          —      49.2M   66.4% [상세]│  │
│ │    73.9M     │     도구 없는 응답  —      49.2M              │  │
│ │              │ ▾ ● 도구          26회    24.5M   33.1% [상세]│  │
│ │  ● 채팅 66%  │     Bash  검증     24회    22.1M              │  │
│ │  ● 도구 33%  │     Read  탐색      2회     2.4M              │  │
│ │  ● MCP  0.5% │ ▾ ● MCP            1회    67.4K    0.5% [상세]│  │
...
조각이나 `[상세]`를 누르면 아래에 **상세 로그**가 열립니다 — 그 범주의 요청들을 JSON 메타데이터로 보여 주는 화면입니다.

### 원장이 아니라 원본을 다시 읽습니다

이 상세만은 SQLite에서 나오지 않습니다. 원장은 요청 단위 집계라 "이 요청이 어떤 도구를 몇 번 불렀나"까지는 알아도 **범주별로 나눈 결과**를 들고 있지 않고, 그것을 넣기 시작하면 원장이 로그의 사본이 됩니다.

그래서 요청이 올 때 원본 파일을 그 자리에서 다시 읽고, 응답만 만들고, 아무것도 저장하지 않습니다. 대가는 셋입니다.

| 대가 | 어떻게 다루나 |
|---|---|
| 원본이 지워지면 상세가 없다 | 지어내지 않고 "원본 없음"으로 답합니다(그 턴은 건너뜁니다). 표의 숫자는 그대로 남습니다 |
| 읽기가 무겁다 | 사람이 그 턴을 열었을 때만 부르고, 한 번에 한 턴만 펼칩니다 |
| 원장과 어긋날 수 있다 | 마지막 스캔 이후 로그가 더 붙을 수 있습니다. 한쪽을 조용히 고르지 않고 **둘 다** 응답에 싣고, 다르면 화면이 그 사실을 적습니다 |

해석은 전부 provider 파서(`parser.mjs`)에 맡깁니다. 파서는 이미 구조 메타데이터만 내보내므로(도구 이름·경로 뒷단·토큰·시각), 상세도 본문에 닿을 곳이 없습니다 — 위 [경계 표](#설계-원칙-본문-없이-절차를-본다)가 그대로 적용됩니다.

### 범주 정의

| 범주 | 무엇 | 토큰 |
|---|---|---|
| 채팅 | 도구를 부르지 않은 요청 | 요청 토큰 전부 |
| 도구 | CLI 내장 도구 호출 — 도구 이름별로 나누고 단계 배지를 붙임 | 호출 비율로 배분 |
| MCP | `mcp__<서버>__<도구>` 모양의 호출 — **서버 → 도구** 두 겹 | 호출 비율로 배분 |
| 파일 | 도구가 만진 파일 경로 뒷단 | **매기지 않음** |

앞의 셋은 서로 겹치지 않고 합이 턴 토큰과 같습니다 — 그래서 원형차트에 들어갑니다. **파일은 도구·MCP 안에서 다시 센 것**이라 같은 원에 넣으면 합이 100%를 넘습니다. 토큰을 매기지 않고 호출 수만 셉니다.

한 요청에 도구가 여럿이면 호출 비율로 나눕니다(단계별 배분과 같은 규칙, 인과가 아니라 배분). 같은 이유로 그런 요청이 만진 파일은 어느 도구가 만졌는지 알 수 없어 `(혼합)`으로 둡니다 — 로그가 말해 주지 않는 것을 골라 주지 않습니다.

### provider별 지원

| provider | 상태 | 근거 |
|---|---|---|
| Claude | 구현 | 도구 이름·경로·턴 경계가 전부 파서에 있음 |
| Codex | 구현 (파일 가지는 비어 있음) | 파서가 경로를 뽑지 않습니다 — **0개가 아니라 미측정**이라 화면이 그렇게 적습니다 |
| Gemini | 템플릿 + TODO | `.json` 전체 스냅샷이 섞여 있어 줄 스캐너가 그대로 들지 않고, 턴 경계가 아직 미확인입니다 |
| Cursor | 템플릿 + TODO | 원본에 대화 구조가 없습니다. "아직 안 만들었다"가 아니라 **만들 근거가 없다** 쪽입니다 |

뒤 둘은 빈 상세가 아니라 `supported: false` 와 이유를 돌려줍니다. 화면이 "이 턴은 아무것도 안 썼다"와 "이 provider는 아직 못 본다"를 구분할 수 있어야 하기 때문입니다. 켜는 순서와 확인할 것은 각 `turn-detail.mjs` 머리말의 TODO에 적어 두었습니다.
...
GET /api/v1/sessions/:sessionId/turns/:turnIndex/detail?provider
    → { turnIndex, ledger, measured, supported, available, reason,
        categories: [{ key, label, tokens, calls, share, pie,
                       children: [{ label, tokens, calls, phase, children }] }],
        files:   [{ path, calls, tools }],
        records: [{ seq, kind, at, category, model, requestId, tokens, tools, paths }],
        source:  { files: [{ path, exists, lines, bytes }], missing, total } }
```
...
getTurnSource({ provider, sessionId, turnIndex })  // 그 턴에 기여한 원본 파일 + 원장 합계
```
...
- [x] 세션 → 프로젝트 이동 시 해당 프로젝트가 선택된 상태로 열림 (선택은 잠기지 않고, 그 프로젝트
      **이름이 검색 필터에** 걸립니다 — `project.md`)
- [x] 프로젝트 → 세션 흐름 이동 시 그 프로젝트로 필터된 상태로 열림
- [ ] Cursor처럼 턴이 불가한 provider는 "미제공"으로 표기 (M6에서 확인)

턴 상세는 `test/turn-detail.test.mjs` 가 따로 검사합니다.

- [x] 범주(채팅·도구·MCP) 합계가 원장의 턴 합계와 일치
- [x] MCP 이름에서 서버와 도구가 갈리고, 서버 이름의 밑줄에 속지 않음
- [x] 같은 요청이 여러 줄·여러 파일에 있어도 상세 합계가 부풀지 않음
- [x] 응답에 프롬프트·도구 입력·응답 본문이 없음 (센티넬 전수 검사)
- [x] 원본 파일이 사라지면 지어내지 않고 `available: false, reason: source_missing`
- [x] 가림된 프로젝트에서는 경로 문자열이 응답에 나가지 않음 (토큰·도구 내역은 유지)
- [x] 아직 붙이지 않은 provider 는 빈 상세가 아니라 `supported: false` 와 이유
- [ ] Gemini·Cursor 실제 연결 (각 `turn-detail.mjs` 의 TODO)
...
- 턴 상세를 원장에 저장하기 — 요청 때 원본을 읽어 응답만 만듭니다. 저장하면 원장이 로그의 사본이 됩니다
- 파일 범주에 토큰 매기기 — 도구·MCP 안에서 다시 센 것이라 파이의 합이 100%를 넘습니다
