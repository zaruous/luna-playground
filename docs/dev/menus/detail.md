...
| 조건 | 규칙 |
|---|---|
| 기본은 닫힘 | 목록(records)·집계 응답에는 본문이 없습니다. 센티넬 검사가 그대로 지킵니다 |
...
| 내용 보기 | 도구 호출이 있는 행의 **[보기]** 버튼. 그 호출의 input·결과 본문을 전용 엔드포인트로 읽어 뷰어에 표시 ([내용 보기 절](#내용-보기-명시적-열람만-저장은-없음)의 조건대로) |
...
GET /api/v1/sessions/:sessionId/tool-calls/:toolUseId/content?provider
    → { supported, available, reason,
        ...
        source: { path, line } }
    -- 본문이 나가는 유일한 통로. Cache-Control: no-store, 저장 없음,
