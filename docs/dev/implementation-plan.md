...
**M6a와 M6b로 분기(2026-08-25).** "원격 사용량은 배제하고 로컬 토큰량 측정
기준으로" 적용해 달라는 요청을 받아, 아래 서술(M6b — Admin API 우선)은 그대로
두고 로컬 전용 트랙을 [cursor/README.md](./cursor/README.md)에 별도로 세웠습니다.
실측 결과 로컬 저장소에는 귀속·요청 수는 있지만 토큰 수 필드가 없어([cursor/
measurements.md](./cursor/measurements.md)), M6a는 "토큰 막대를 채운다"가 아니라
"있는 신호만 정직하게 보여준다"는 작업입니다. 자격증명·레이트리밋·시간창
커서 인프라는 M6a에는 필요 없습니다(원격 호출이 없으므로). 아래는 M6b로 남깁니다.
