...
### Phase 0b 감도 실측 (2026-08-26) — 카테고리 선택은 문제없음, 압축 클램프는 필수

"다른 계산 방식으로 토큰량 자체는 구할 수 있는가"라는 질문에, 대화별로
`snapshot(N) - snapshot(N-1)`을 이어붙이는 방식(음수는 0으로 클램프)을 실제로
`scripts/probe-cursor-delta.mjs`로 돌려 감도를 쟀습니다. 대상은 CLI 대화
32개(파일 하나 = 대화 하나로 자연히 묶여 순서를 매길 수 있음) — IDE 쪽
(`cursorDiskKV`)은 blob이 content-addressed라 대화 귀속 없이는 순서를 못 매겨
이번 패스 스코프 밖입니다.

**① 카테고리 선택 민감도 — 0이었습니다(정정).** `5.1`(전체) 기준 클램프 합계와
`conversation` 카테고리만 기준으로 한 클램프 합계가 31개 대화·554개 구간 전부에서
**완전히 동일**(1.00배)했습니다. 원인: `system_prompt`/`tools`/`rules`/`skills`/
`mcp`/`subagents`는 관측된 모든 대화에서 **시작 시 한 번 정해지면 그 대화 안에서
다시 안 바뀝니다**. 그러니 두 방식 중 뭘 써도 결과가 같습니다 — 앞서 "conversation
전용이 더 안전하다"고 했던 우려는 이 표본에서는 근거가 없었습니다.

**② 압축(compaction) — 드물지만 크기가 큽니다.** 554개 구간 중 음수 델타
**1건(0.18%)**, 31개 대화 중 **1개**에서만 발생. 그 1건의 크기가 -117,844
토큰으로 클램프 후 전체 합계(1,009,463)의 **11.67%**에 해당합니다. 실측으로
직접 확인한 그 지점(대화 `eca64170-...`, CLI, window 200000):

```text
압축 전(3개 연속 스냅샷, rowid 1021/1024/1035, 5.1=186654, 창의 93.3%):
  system_prompt 517 · tools 8079 · rules 3026 · skills 2255 · mcp 1039 ·
  subagents 503 · summarized_conversation 0 · conversation 171235

압축 후(2개 연속 스냅샷, rowid 1051/1054, 5.1=68810, 창의 34.4%):
  system_prompt 517 · tools 8079 · rules 3026 · skills 2255 · mcp 1039 ·
  subagents 503 · summarized_conversation 4986 · conversation 48405
```

`conversation`이 171,235 → 48,405로 줄고 `summarized_conversation`이 0 → 4,986으로
채워졌습니다 — 즉 이전 대화 내용 중 122,830 토큰어치가 요약 4,986 토큰으로
압축되고(≈34:1), 최근 48,405 토큰만 원문 그대로 남았습니다. 창 크기(200,000)의
93%까지 찬 시점에 발생했다는 것도 "토큰 상한에 가까워지면 압축이 트리거된다"는
가설과 일치합니다. 세부 해석과 "압축이 서버 전송량에 미치는 영향"은
[커서컨텍스트관리.md](./커서컨텍스트관리.md)에 정리했습니다.

**③ 부수 확인 — 중복 관측 패턴, 다만 정확히 2개는 아니었습니다.** 결정 3번의
"같은 값이 두 blob에 연속으로 나타난다"는 대체로 맞지만, 위 압축 전 구간에서는
**3개** 연속으로 같은 값이 나온 사례도 있었습니다 — "정확히 2개"라고 단정할 근거는
없고 "짧게 여러 번 반복된다" 정도로 정정합니다. "최신 관측값 승리" 규칙 자체는
이번에도 깨지지 않았습니다(반복된 값끼리는 항상 동일).

**남는 한계**: 표본이 이 계정의 32개 CLI 대화뿐이라 "압축이 몇 %로 일어나는지"는
신뢰구간을 못 냅니다. 더 긴 대화가 많은 계정에서는 11.67%보다 훨씬 커질 수 있고,
IDE 쪽(`cursorDiskKV`)은 이번 패스에서 아예 안 봤습니다 — 둘 다 미확인으로
남깁니다.

재현: `node scripts/probe-cursor-delta.mjs`.
