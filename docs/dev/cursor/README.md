...
바뀌는지**를 중심으로 정리합니다. 화면/기능 하나하나가 "가능/보류/배제" 중
무엇인지 한 표로만 보려면 [기능적용가능성.md](./기능적용가능성.md)를 보세요 — 이
문서의 "메뉴별 적용 계획" 절과 같은 판단을 표로 압축한 것입니다. 다른 세
provider와 나란히 놓고 보려면 [LLM별 기능지원표.md](../LLM별%20기능지원표.md)입니다.
...
### `usage` (AI 사용량) — 완료 ([usage.md T2](./menus/usage.md#t2-cursor-컨텍스트-구성-반영--완료))

- 기간별 **토큰 누적 막대 차트**(요청 델타 기반)에는 계획대로 **Cursor를 넣지
  않았습니다** — 그 차트의 단위(요청이 쓴 입력/출력)와 breakdown의 단위(그 시점
  컨텍스트 구성)가 달라서 같은 막대에 합치면 둘 다 거짓말이 됩니다.
- 대신 이 화면 하단에 별도 패널 **"Cursor — 컨텍스트 구성"**을 추가했습니다:
  기간 내 대화(컴포저)별 마지막 관측 breakdown을 카테고리(system_prompt/tools/rules/
  skills/mcp/subagents/summarized_conversation/conversation) 스택 막대로,
  최신 20개까지·잘린 개수와 IDE 제외 개수를 안내 문구로. 카테고리 색은 새로
  검증한 `cx-*` 8색 팔레트(`src/styles.css`, `dataviz` 스킬 검증). 기존
  `getUsageTimeseries`와는 분리된 엔드포인트
  `GET /api/v1/cursor/context?since=&until=&all=`로 — 계획 당시 적었던
  `since&until`에 `all` 플래그가 이 화면의 기존 관례(§API)를 따라 추가됐습니다.
  응답은 `tokens: {...}` 요청 델타 모양이 아니라 컴포저별 "마지막 관측"
  breakdown 나열이라 기존 시계열 계약과 섞이지 않습니다.
- 모델 breakdown 차트에는 Cursor를 계획대로 제외(요청 단위 모델별 토큰이 아직 없으므로).

### `project` (프로젝트) — 완료 ([project.md T3](./menus/project.md#t3-cursor-프로젝트-반영--완료))

- 프로젝트 목록/상세에 Cursor 프로젝트가 합류합니다 — `getProjectBreakdown`이
  `usage_events` 정렬 결과 뒤에 `cursor_local_activity` 집계를 **append**하는
  방식으로(interleave 아님, 토큰 정렬 순서를 깨지 않기 위해). 세션 수 열은
  Cursor 행에서 "대화" 라벨로 컴포저 수를 보여줍니다(다른 provider의 "세션"과
  다른 단위임을 라벨로 구분).
- 프로젝트 상세의 **요청 델타 "토큰" 카드**에는 Cursor 몫이 **빠집니다**(`null`,
  0으로 섞지 않음). 대신 새 "Cursor 활동" 카드: 요청 수, 마지막 관측
  breakdown(카테고리별 토큰 미니 막대), 변경 라인 — `getCursorProjectActivity()`.
- 프로젝트 가림(redaction) 규칙은 그대로 적용됩니다 — `cwd`가 가려지면 Cursor
  행도 같은 별칭을 씁니다(기존 `#applyProjectPrivacy` 재사용).
- 구현 중 찾은 잠재 결함도 같이 고쳤습니다: `#resolveProjectKey`가 기존에
  `usage_events`만 스캔해 Cursor `projectKey`를 클릭하면 404였던 것을,
  `cursor_local_activity`의 distinct 프로젝트명도 함께 스캔하도록 확장.
- 이 구현을 검증한 적대적 리뷰 Workflow(review→verify 2단계)가 cwd 없는
  컴포저("(미분류)" 버킷)에서 같은 종류의 결함 둘을 추가로 찾았고, 모두
  고쳤습니다 — [project.md T4](./menus/project.md#t4-cursor-미분류-버킷-정합성--완료).
...
- "한도 이력"·"대조 타임라인"은 **서버 대조가 전제**인 화면입니다. Cursor는
  `serverQuota: false`이므로 계획대로 이 화면에는 안 나타납니다.
  **진단 패널만 완료** — 계획했던 문장형 한 줄 대신, 다른 다섯 카운터와 나란한
  여섯 번째 `.stat-mini` 카운터(`cursorActivity`, `cursor_local_activity` 행 수)로
  더 단순하게 구현했습니다(`getDiagnostics()`, [budget.md](./menus/budget.md#진단)).
  마지막 관측 컨텍스트 총 토큰/창 크기 문구는 아직 없습니다 — 대시보드 provider
  카드가 이미 그 값을 보여주므로 중복 없이 다음 패스로 미룹니다.
...
- [x] `cwd`/`trackedGitRepos` 기반 프로젝트 귀속이 프로젝트 화면에 반영되고,
      가림 설정이 다른 provider와 동일하게 적용됨 — "최근 프로젝트 발자국"(대시보드)
      뿐 아니라 `ProjectView.jsx`의 프로젝트 목록·상세도 반영됨
      ([project.md T3](./menus/project.md#t3-cursor-프로젝트-반영--완료)),
      `#applyProjectPrivacy` 재사용으로 가림 규칙 동일 적용,
      `#resolveProjectKey`도 `cursor_local_activity`를 스캔하도록 확장해
      Cursor `projectKey` 클릭이 404 나지 않음
