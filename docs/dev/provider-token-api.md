...
| 항목 | 값 |
|---|---|
| 원본 | `~/.claude/projects/<project>/<conversation-id>.jsonl` |
| 토큰 필드 | `usage.input_tokens`, `usage.output_tokens`, `usage.cache_read_input_tokens`, `usage.cache_creation_input_tokens` |
| 증분 방식 | **레코드가 이미 요청 단위** — 누적 diff가 아니라 요청별 값 |
| 중복 제거 | `message.id` + `requestId`, **last-wins** (R1) |
...
**(a) 스트리밍 중간 기록** — 같은 `requestId`의 후행 레코드가 최종값입니다. byte offset tail 방식과 충돌하는데, 이미 커밋한 값을 나중 값으로 교체해야 하기 때문입니다. 해결:

```js
// 저장은 (provider, event_key) UPSERT — 마지막 관측이 이긴다
// event_key = `claude|${messageId}|${requestId}`
store.upsertUsageEvent(event);   // 신규 메서드. 기존 insert-if-absent와 별도
```

즉 Claude 어댑터는 **append-only 가정을 버리고 요청 단위 upsert**를 씁니다. `usage_events`에 이미 `event_key`가 있으므로 스키마 변경 없이 UNIQUE 제약과 UPSERT만 추가합니다([store-extensions.md](./store-extensions.md)).

**(b) 필드별 신뢰도** — R2에 따라 그대로 노출합니다.

| 필드 | 등급 | 근거 |
|---|---|---|
| `cachedInputTokens` | `local_exact` | 상태줄과 일치 |
| `cacheWriteInputTokens` | `local_exact` | 동일 |
| `inputTokens` | `unverified` | 항목 75%가 0/1 플레이스홀더 |
| `outputTokens` | `partial` | thinking 미포함 |
| `reasoningTokens` | 미기록 | 로그에 필드 자체가 없음 → 0이 아니라 null (R7) |

따라서 이벤트 `measurementQuality`는 `unverified`이고, 대시보드 총합에는 "미확인" 배지가 붙습니다. **여기서 임의 보정 계수를 곱하지 않습니다.**

**(c) 선택적 텔레메트리 보강** — `capabilities.telemetry = true`. 사용자가 켜면 서비스가 로컬 OTLP 수신 지점을 열고, Claude Code에 다음을 설정하도록 안내합니다.
...
|---|---|---|---|---|---|
| `inputTokens` | `input_tokens` | `usage.input_tokens` ⚠ | `type=input` | `tokenUsage.inputTokens` | input |
| `cachedInputTokens` | `cached_input_tokens` | `usage.cache_read_input_tokens` | `type=cacheRead` | `tokenUsage.cacheReadTokens` | cached |
| `cacheWriteInputTokens` | — | `usage.cache_creation_input_tokens` | `type=cacheCreation` | `tokenUsage.cacheWriteTokens` | — |
...
| `toolTokens` | — | — | — | — | tool |
| `totalTokens` | `total_tokens` | 합산 | 합산 | 합산 | total |
